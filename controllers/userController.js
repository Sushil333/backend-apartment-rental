...
export const signin = asyncHandler(async (req, res) => {
  ...
  if (!email && !password)
    return res.status(400).json({ data: "Email and Password is required!" });
  ...
  if (!oldUser) return res.status(404).json({ data: "User doesn't exist" });
  if (!oldUser.active) res.status(400).json({ data: "Account is blocked!" });
...
export const signup = asyncHandler(async (req, res) => {
  ...
  // check for user existance
  const oldUser = await User.findOne({ email });
  if (oldUser) return res.status(400).json({ data: "User already exists" });
